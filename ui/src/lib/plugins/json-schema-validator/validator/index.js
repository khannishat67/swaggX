...
export default class JSONSchemaValidator {
  ...
  validateSwaggar2Spec(schema, key, { jsSpec, specStr, schemaPath, source }) {
    this.addSchema(schema, key);
    return this.validate({ jsSpec, specStr, schemaPath, source });
  }
  ...
  validate({ jsSpec, specStr, schemaPath, source }) {
    ...
    try {
      ...
      return {
        level: "error",
        line: err.problem_mark && err.problem_mark.line + 1 || 0, //eslint-disable-line
        message: err.problem,
        source: "parser",
        original: err
      }
